...
function addDetectorMedia(tabId, rawMedia, pageTitle) {
  ...
  const variants = playlist.map((entry, index) => {
    ...
    try {
      resolvedVideoUrl = new URL(String(videoUrl), safeUrlObject(url)).href;
    } catch (err) {
      console.warn('[GrabIt] Failed to resolve video URL:', err.message, {
        videoUrl,
        url
      });
    ...
    try {
      if (audioUrl) resolvedAudioUrl = new URL(String(audioUrl), safeUrlObject(url)).href;
    } catch (err) {
      console.warn('[GrabIt] Failed to resolve audio URL:', err.message, {
        audioUrl,
        url
      });
      // Fallback to original string
...
// Tab lifecycle is no longer needed for cookie extraction
...
// -------------------------------------------------------
vdownChannel.addEventListener('message', event => {
// console.log('[VDown MESSAGE]', JSON.stringify(event.data, null, 2));
console.log(
  '[VDown MESSAGE]',
...
if (
  event.data?.msg?.name === 'download_error' ||
  event.data?.msg?.name === 'download_result'
) {
  console.error(
    '[VDown FINAL MESSAGE]',
    JSON.stringify(event.data, null, 2)
...
//     if (message.name === 'download_result') {
...
//           : error?.details
//             ? error.details
//             : JSON.stringify(error) || 'LibAV worker failed'
//     )
//   );
// }
//   }
...
// -------------------------------------------------------
if (
  (message?.name === 'download_result' ||
   message?.name === 'download_error') &&
  id
) {
  const waiter = vdownWaiters.get(id);
  if (!waiter) {
    console.warn('[GrabIt] Received final worker message for unknown download:', {
      downloadId: id,
      message
    });
    return;
  ...
  console.log('[GrabIt] FINAL WORKER RESULT:', {
    downloadId: id,
    name: message.name,
    data: message.data
  });
  ...
  const rawError = message.data?.error;
  let errorText = 'LibAV worker failed.';

  if (typeof rawError === 'string' && rawError.trim()) {
    errorText = rawError;
  } else if (rawError?.message) {
    errorText = rawError.message;
  } else if (rawError?.details) {
    errorText = rawError.details;
  } else if (message.data?.ending_reason) {
    errorText =
      typeof message.data.ending_reason === 'string'
        ? message.data.ending_reason
        : message.data.ending_reason?.message ||
          message.data.ending_reason?.details ||
          JSON.stringify(message.data.ending_reason);
  } else {
    try {
      errorText = JSON.stringify(message.data);
    } catch (stringifyErr) {
      console.error('[GrabIt] Failed to stringify worker message:', stringifyErr);
      // errorText retains its current value from earlier assignments
    }
  }

  console.error('[GrabIt] LibAV worker DOWNLOAD ERROR:', {
    downloadId: id,
    error: errorText,
    rawMessage: message
  });
...
//     throw new Error('The stream worker returned no result.');
//   }

//   if (result.aborted_no_partial) {
//     throw new Error(workerFailure(result));
//   }

//   // Prefer a worker-created Blob URL when available.
//   if (typeof result.internal_bloburl === 'string' && result.internal_bloburl) {
//     return result.internal_bloburl;
//   }

//   // Otherwise use the OPFS filename returned by LibAV.
//   if (typeof result.internal_filename === 'string' && result.internal_filename) {
//     return await opfsBlobUrl(result.internal_filename);
//   }

//   // Give us the actual worker result instead of the useless
//   // "neither a file nor a download URL" message.
//   throw new Error(
//     `Worker completed without a downloadable result: ${JSON.stringify({
//       ending_reason: result.ending_reason ?? null,
//       aborted_no_partial: result.aborted_no_partial ?? false,
//       internal_filename: result.internal_filename ?? null,
//       has_internal_bloburl: Boolean(result.internal_bloburl)
//     })}`
//   );
// }
...
async function workerDownloadUrl(result) {
  if (!result || typeof result !== 'object') {
    throw new Error('The local stream worker returned no result.');
  }
  ...
  // Check for worker-reported failures
  const reason = result?.ending_reason;
  if (reason && (
    reason.user_abort === true ||
    reason.e4XX_5XX_failure === true ||
    reason.percentage_incomplete === true ||
    reason.other_failure === true
  )) {
    // Use the specific error message from the worker if available
    const errorMessage = reason.message || reason.details || 'The stream worker reported a failure.';
    throw new Error(`Stream worker failed: ${errorMessage}`);
  }

  // PRIMARY PATH:
  // Use the OPFS file and let the durable offscreen document
  // create the Blob URL.
  if (
    typeof result.internal_filename === 'string' &&
    result.internal_filename.trim()
  ) {
    console.log('[GrabIt] Using OPFS output:', result.internal_filename);

    return await opfsBlobUrl(result.internal_filename);
  }

  // FALLBACK:
  // Some worker strategies may return only a Blob URL.
  if (
    typeof result.internal_bloburl === 'string' &&
    result.internal_bloburl
  ) {
    console.log('[GrabIt] Using worker Blob URL');

    return result.internal_bloburl;
  }

  throw new Error(
    `The stream worker completed without a downloadable file. ${JSON.stringify({
      ending_reason: result.ending_reason ?? null,
      internal_filename: result.internal_filename ?? null,
      has_internal_bloburl: Boolean(result.internal_bloburl)
    })}`
...
async function opfsBlobUrl(filename) {
  if (typeof filename !== 'string' || !filename.trim()) {
    throw new Error(
      'The stream worker finished without returning an OPFS filename.'
    );
  }
  ...
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      opfsBlobWaiters.delete(requestId);
      reject(
        new Error(
          `Timed out while converting the worker file to a Blob URL: ${filename}`
        )
      );
    }, 15_000);
    ...
    opfsBlobWaiters.set(requestId, {
      resolve: url => {
        ...
        if (!url) {
          reject(new Error('The OPFS bridge returned an empty Blob URL.'));
          return;
        }
    ...
    vdownChannel.postMessage({
      channel: 2,
      msg: {
        name: 'create_blob_url_from_file',
        data: {
...
// Gets cookies for a URL and returns them as a cookie string suitable for HTTP headers
async function getCookiesForUrl(url) {
  try {
    ...
    // Format as cookie string, filtering for path relevance
    return cookies
      .filter(cookie =>
        // Only include cookies that are relevant to this URL path
        cookie.path === '/' || urlObj.pathname.startsWith(cookie.path)
      )
      .map(cookie => `${encodeURIComponent(cookie.name)}=${encodeURIComponent(cookie.value)}`)
      .join('; ');
  } catch (error) {
    console.warn('[GrabIt] Failed to get cookies for URL:', url, error);
    return '';
  }
}
//   let extension, workerBlobUrl = null;
//   console.log('[GrabIt] OFFLINE STREAM INPUT', {
//     tabId,
//     streamUrl,
...
//     streamVariants: stream.variants,
//     hasAudioUrl: Boolean(variant?.audioUrl)
// });
...
//       will_use_jsfetch: true,strategy: 'http_audio_video_one_source_jsfetch', url: new URL(stream.url), carry_get_params: false,
...
//                 data: {
//                     blob_url: workerBlobUrl
//                 }
//             }
//         });
//     } catch (error) {
//         console.warn(
//             '[GrabIt] Failed to request Blob URL cleanup:',
//             error
//         );
//     }
// }, 60_000);
...
async function offlineStreamDownload(tabId, streamUrl, variantId) {
  ...
  if (!stream) {
    throw new Error(
      'The selected stream is no longer available. Refresh the page and try again.'
    );
  }

  const variant =
    stream.variants?.find(v => v.id === variantId) || null;
  ...
  const report = progress => {
    const percent = progress.percent?.is_known
      ? Math.round(progress.percent.value)
      : (
          progress.total
            ? Math.round(
                ((progress.completed || 0) * 100) / progress.total
              )
            : undefined
        );
  ...
  notifyStreamProgress(tabId, streamUrl, {
    phase: 'starting'
  });
  ...
  try {
    ...
    if (stream.type === 'HLS') {
      if (!variant) {
        throw new Error(
          'The HLS qualities are still loading. Please try again in a moment.'
        );
      }
      ...
      const downloadArgs = {
        ...
        ...(variant.audioUrl
          ? {
              url_audio: new URL(variant.audioUrl)
            }
          : {}),
      ...
      workerBlobUrl =
        await workerDownloadUrl(result);
    ...
    // ============================================================
      ...
      if (!variant) {
        throw new Error(
          'The DASH qualities are still loading. Please try again in a moment.'
        );
      }
    ...
    if (!workerBlobUrl) {
      throw new Error(
        'The local stream worker did not return a downloadable file.'
      );
    }
    ...
    try {
      updateOfflineJob(jobId, {
        progress: {
          percent: 100,
          detail:
            'Saving to Downloads/Streams…'
        }
      });

      const browserDownloadId =
        await chrome.downloads.download({
          url: workerBlobUrl,
          filename:
            `Streams/${safeFilename(
              stream.name,
              extension
            )}`,
...
// ── Startup ───────────────────────────────────────────────────────────────────
console.log('[GrabIt] Background service worker active — stream detection enabled');