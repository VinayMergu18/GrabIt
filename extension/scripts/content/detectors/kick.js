      ...
      this.tagMappers
        ...
        .forEach((n) => {
          ...
          if (((t = /^#EXT-X-BYTERANGE:(.*)?$/.exec(n)), t)) {
            ((e = V(me(t[1]), { type: "tag", tagType: "byterange" })),
              this.trigger("data", e));
            return;
          }
...
function Xe() {
  ...
  for (let r of i) {
    ...
    t.isSome() &&
      ...
      ze(t.value.id ?? t.value.channel, t.value.type);
  }
}
function Be() {
  let i = Ve(window.location.href);
  if (i.isSome()) {
    let r = i.value.type == "channel";
    r || (!r && i.value.id)
      ? ze(i.value.id ?? i.value.channel, i.value.type)
      : he ||
        (Xe(),
