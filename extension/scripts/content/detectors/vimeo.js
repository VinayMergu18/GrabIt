  ...
  if ((r && typeof r == "object") || typeof r == "function")
    for (let a of Ue(r))
      !Ve.call(i, a) &&
        ...
        ce(i, a, {
          get: () => r[a],
          enumerable: !(e = Fe(r, a)) || e.enumerable,
        });
  return i;
};
var he = (i, r, t) => (
...
var De = pe((fe, Me) => {
  (function (i, r) {
    ...
    else if (typeof fe < "u") r(Me);
    else {
      var t = { exports: {} };
      (r(t), (i.browser = t.exports));
    }
  })(
    typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : fe,
    function (i) {
      "use strict";
      if (!(
        globalThis.chrome &&
        ...
        let r = "The message port closed before a response was received.",
          t = (e) => {
            ...
            let u = (d) =>
              ...
              o = (d, f) =>
                function (_, ...R) {
                  if (R.length < f.minArgs)
                    throw new Error(
                      `Expected at least ${f.minArgs} ${g(f.minArgs)} for ${d}(), got ${R.length}`,
                    );
                  if (R.length > f.maxArgs)
                    throw new Error(
                      `Expected at most ${f.maxArgs} ${g(f.maxArgs)} for ${d}(), got ${R.length}`,
                    );
                  return new Promise((M, P) => {
                    if (f.fallbackToNoCallback)
                      try {
                        _[d](...R, l({ resolve: M, reject: P }, f));
                      } catch (y) {
                        (console.warn(
              ...
              T = (d, f, p, ..._) => {
                ...
                return new Promise((R, M) => {
                  let P = C.bind(null, { resolve: R, reject: M });
...
(function (i) {
  function r(l) {
    ...
    for (
      var h = l === void 0 ? [] : Array.isArray(l) ? l : H([l], g, !0),
        A = [],
        E = 0,
        S = h;
      E < S.length;
  ...
  function a(l) {
    try {
      ...
      return Promise.resolve(new I(g));
    }
  }
  i.wrapAsync = a;
  function n(l) {
  ...
  i.partition = n;
  function u(l) {
    return l instanceof I || l instanceof D;
  }
  i.isResult = u;
})(K || (K = {}));
...
function ye(i) {
  for (let r of je) if (r.mime_reg.test(i)) return x(r.demuxer);
  return b;
}
function Ke(i, r) {
...
function Ae(i) {
  return [...i.values()].sort((r, t) => Ke(r.quality, t.quality));
}
function _e(i) {
  return i.length > 0;
}
      ...
      this.tagMappers
        ...
        .forEach((n) => {
          ...
          if (((t = /^#EXT-X-BYTERANGE:(.*)?$/.exec(n)), t)) {
            ((e = z(ue(t[1]), { type: "tag", tagType: "byterange" })),
              this.trigger("data", e));
            return;
          }
            ...
            tag() {
              (
                ({
                  ...
                  "server-control"() {
                    let c = (this.manifest.serverControl = V(s.attributes));
                  ...
                  daterange() {
                    ...
                    if (!v[m.id]) v[m.id] = m;
                      ...
                      for (let p in v[m.id])
                        if (
                          m[p] &&
                          JSON.stringify(v[m.id][p]) !== JSON.stringify(m[p])
                        ) {
                          this.trigger("warn", {
                            message:
...
function Se(i, r, t) {
  ...
  for (let n of r) {
    ...
    let l = e.get(u);
    if (l) return x(l);
    for (let [g, o] of e) if (g.split("-")[0] === u) return x(o);
  }
  return b;
}
var tr = (() => {
...
function we(i, r, t, e = !1, a = !1) {
  ...
  for (let o of u) {
    ...
    if (o.attributes.AUDIO) {
      ...
      if (T) {
        if (t.isSome()) {
          let c = Se(Object.values(T), t.value, (m) => m.language ?? "");
          if (c.isSome()) E = U(c.value.uri, r.href);
          else {
            let m = Object.values(T).find((d) => d.default);
            E = m ? U(m.uri, r.href) : b;
          }
        } else
          for (let c of Object.values(T))
            if ((c.uri && (E = U(c.uri, r.href)), c.default)) break;
      }
    }
    let S = b,
      v = b;
    (o.attributes.RESOLUTION &&
      ...
      o.attributes.BANDWIDTH && (v = x(o.attributes.BANDWIDTH)));
    let s = { size: S, bitrate: v },
      C = A.value;
    if (
      (e
        ? (C.search = ae(C.searchParams, r.searchParams))
        : a && (C.search = ""),
      E.isSome())
    ) {
      let T = E.value;
      (e
        ? (T.search = ae(C.searchParams, r.searchParams))
        : a && (T.search = ""),
        g.push({ demuxer: h, quality: s, av: { video: C, audio: T } }));
    } else g.push({ demuxer: h, quality: s, av: { video: C, audio: !1 } });
  }
  return ((g = Ae(g)), _e(g) ? D(g) : I("Empty playlist"));
}
async function Ie(i) {
...
var _r = new BroadcastChannel("worker_service");
var B = {
...
var ge = new BroadcastChannel(`injected-${ee(window.location.href)}`);
function Re(i) {
  let r = B.FromTrustedInjectedToUntrusted;
  ge.postMessage({ msg: i, channel: r });
}
function Oe(i) {
  let r = (t) => {
    let e = t.data.msg;
    t.data.channel == B.FromUntrustedInjectedToTrusted && i(e);
  };
  return (
    ge.addEventListener("message", r),
    () => {
      ge.removeEventListener("message", r);
    }
  );
}
var te = he(De(), 1);
async function ut(i, r) {
  await te.default.runtime.sendMessage({ msg: i, channel: r });
}
function Pe(i) {
  let r = B.FromInjectedToService;
  ut(i, r);
}
function Ne(i) {
  let r = (t) => {
    t.channel == B.FromServiceToInjected && i(t.msg);
  };
  return (
    te.default.runtime.onMessage.addListener(r),
    () => {
      te.default.runtime.onMessage.removeListener(r);
    }
  );
}
async function Ce(i) {
  let r = b,
    t = i.video?.thumbs?.base;
  (t || (t = i.video?.thumbnail_url), t && (r = U(t)));
  let e = b,
    a = i.video?.title;
  a && (e = x(a));
  let n = b,
    u = i.request?.files?.hls?.cdns,
    l = i.request?.files?.hls?.default_cdn;
  if ((u && l in u && (n = U(u[l].url)), n.isSome())) {
    let g = n.value,
      o = await fetch(g);
    if (o.ok) {
      let h = await o.text(),
...
Ne((i) => {
  i.name == "vimeo_on_config" && Ce(i.data);
});
Re({ name: "vimeo_request_config", data: null });
var mt = Oe((i) => {
  i.name == "vimeo_on_config" && (Ce(i.data.config), mt());
});
