...
async function getYouTubeCookieStoreId() {
  ...
  // Chrome: get cookie stores and find matching tabId
  const stores = await chrome.cookies.getAllCookieStores();
  return stores.find(s => s.tabIds.includes(tab.id))?.id;
}

// Get cookies for youtube.com and format as Netscape cookie file string
async function getYouTubeCookies() {
  try {
    const storeId = await getYouTubeCookieStoreId();
    const details = { url: 'https://www.youtube.com' };
    if (storeId) {
      details.storeId = storeId;
    }
    const cookies = await chrome.cookies.getAll(details);
    if (!cookies || cookies.length === 0) return '';
    // Build Netscape format lines
    const lines = [
    ...
    for (const c of cookies) {
      ...
      // Ensure no tab characters in name/value (replace with space)
      const name = c.name.replace(/\t/g, ' ');
      const value = c.value.replace(/\t/g, ' ');
      const line = [domain, includeSubDomain, path, secure, expires, name, value].join('\t');
      lines.push(line);
    }
    lines.push(''); // trailing newline
    return lines.join('\n');
  } catch (err) {
    console.error('Failed to get YouTube cookies:', err);
    return '';
  }
}

// Get cookies for instagram.com and format as Netscape cookie file string
async function getInstagramCookies() {
  try {
    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
    if (!tab) return '';

    let details = { url: 'https://www.instagram.com' };
    ...
    console.error('Failed to get Instagram cookies:', err);
    return '';
  }
}

// Get cookies for current tab and format as Netscape cookie file string
async function getCurrentTabCookies() {
  try {
    ...
    console.error('Failed to get current tab cookies:', err);
    return '';
  }
}

// Get all cookies from browser and format as Netscape cookie file string
async function getAllCookiesForExport() {
  try {
    // Helper: get current cookie store ID (handles split incognito)
    ...
    // Get all cookies (including partitioned) using the cookie extractor logic
    ...
    const cookies = await getAllCookies({}); // no filter -> all cookies
    if (!cookies || cookies.length === 0) return '';

    // Netscape format mapper (copied from cookie-format.mjs)
    const jsonToNetscapeMapper = (cookiesArr) => {
      return cookiesArr.map(
    ...
    console.error('Failed to get all cookies:', err);
    return '';
  }
}
...
function renderYTFull(probe, url) {
  ...
  if (!isPlaylist) {
    // Set up subtitle language selector for video+subs panel
    const subsPanel = document.getElementById('yt-panel-subs');
    if (subsPanel) {
...
function renderYTVideoTabs(probe) {
  ...
  const isPlaylist = probe.contentType === 'playlist' || probe.contentType === 'mix_playlist';
  const hasSubs = !isPlaylist && (subInfo.hasSubtitles || probe.hasSubtitles);
  const langCount = !isPlaylist ? (subInfo.languages || probe.subtitleLangs || []).length : 0;

  return `
    <div class="yt-tab-bar">
      <button class="yt-tab active" data-yttab="video">📹 Video</button>
      <button class="yt-tab" data-yttab="subs"
        title="${hasSubs ? 'Download video with embedded subtitles' : 'No subtitles detected'}">
...
function dedupeByHeight(list) {
  ...
  for (const f of list) {
    const key = f.height || 'best';
    const existing = byHeight.get(key);
    if (!existing || (f.size || 0) > (existing.size || 0)) byHeight.set(key, f);
  }
  return Array.from(byHeight.values());
}

function renderYTVideoPanel(probe) {
  let formats = (probe.videoFormats || []).slice();
  formats = dedupeByHeight(formats);
  // Sort by height descending
  formats.sort((a, b) => (b.height || 0) - (a.height || 0));
  if (formats.length === 0) return '<div style="padding:10px;color:var(--text3);font-size:12px">No video formats detected</div>';

  const rows = formats.map((f, i) => `
    <button class="yt-quality-row${!f.available ? ' yt-q-disabled' : ''}"
      ${f.available ? `data-yt-action="download_video" data-quality="${f.height || 'best'}"` : 'disabled'}>
      <span class="yt-q-idx">[${i}]</span>
      <span class="yt-q-label">${f.height}p</span>
...
function renderYTSubsPanel(probe) {
  ...
  // If playlist, show a message that this feature is not available
  if (isPlaylist) {
    return `<div style="padding:14px 10px;color:var(--text3);font-size:12px;text-align:center">
      Video+subtitles download is not available for playlists.<br>
      Use the Video tab to download individual videos with subtitles.
    </div>`;
  }
  ...
  const subsOnlyRow = `
    <button class="yt-quality-row yt-q-special" data-yt-action="download_subs_only" data-quality="best">
      <span class="yt-q-idx">[${formats.length}]</span>
...
function wireYTTabs(probe, url) {
  ...
  document.querySelectorAll('[data-yt-action]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      // If subtitle-related action, add selected subtitle language
      if (action === 'download_video_subs' || action === 'download_subs_only') {
        const lang = state.ytSubtitleLang;
        if (lang) {
          opts.subtitleLang = lang;
        }
      }
...
function renderSettings() {
        ...
        <div class="settings-row">
          ...
          <div class="radio-group">
            <label class="radio-option">
              <input type="radio" name="cookieSource" value="auto" ${app.cookieSource === 'auto' || app.cookieSource === undefined ? 'checked' : ''}>
              <span>Auto-extract from browser</span>
            </label>
            <label class="radio-option">
              <input type="radio" name="cookieSource" value="file" ${app.cookieSource === 'file' ? 'checked' : ''}>
              <span>Use cookies from file</span>
            </label>
          </div>
        </div>
        <div class="settings-row" id="cookie-file-path-row" ${app.cookieSource === 'file' ? '' : 'style="display:none;"'}>
          <div class="settings-label">Cookie file path</div>
          <input class="settings-input" data-section="app" data-key="cookieFilePath" value="${app.cookieFilePath || ''}" placeholder="path to cookies.txt">
        </div>
  ...
  // Cookie source radio button listeners
  ...
  cookieSourceRadios.forEach(radio => {
    radio.addEventListener('change', () => {
      ...
      document.getElementById('cookie-file-path-row').style.display = isFileMode ? '' : 'none';
      document.getElementById('cookie-extract-row').style.display = !isFileMode ? '' : 'none';
    });
  });

  // Cookie extract button listener
  const extractBtn = document.getElementById('extract-cookies-btn');
  if (extractBtn) {
    extractBtn.addEventListener('click', async () => {
      extractBtn.disabled = true;
      extractBtn.innerHTML = '<span class="button-text">Extracting...</span>';
      const statusEl = document.getElementById('cookie-extract-status');
      if (statusEl) {
      ...
      try {
        ...
        if (result.ok) {
          ...
          if (statusEl) {
            ...
            if (result.cookieFilePath) {
              const cookieFilePathInput = document.getElementById('cookie-file-path');
              if (cookieFilePathInput) {
                cookieFilePathInput.value = result.cookieFilePath;

                // Save the updated file path to settings
                try {
                  await api('PATCH', '/settings/app', {
                    cookieFilePath: result.cookieFilePath
                  });
                } catch (e) {
                  console.warn('Could not save cookie file path:', e);
                }
              }
            }
          }
        } else {
          throw new Error(result.error || 'Failed to extract cookies');
        }
      } catch (err) {
        console.error('Cookie extract error:', err);
        if (statusEl) {
        ...
        extractBtn.innerHTML = '<span class="button-text">Extract Cookies Now</span>';
...
async function init() {
  ...
  if (online) {
    ...
    try {
      ...
      // Initialize subtitle language state
      state.ytSubtitleLang = undefined;
    } catch {}
